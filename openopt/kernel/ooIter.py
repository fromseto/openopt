...
def ooIter(p, *args,  **kwargs):
    ...
    if not p.isFinished or len(p.iterValues.f) == 0:
        ...
        if p.isManagerUsed:
            if p.iterTime[-1] - p._manager_time > 1:
                p._manager_time_variable.set('%d' % int(p.iterTime[-1]))
                p._manager_time = p.iterTime[-1]
                if p.iterCPUTime[-1] - p._manager_cputime > 1:
                    p._manager_cputime_variable.set('%d' % int(p.iterCPUTime[-1]))
                    p._manager_cputime = p.iterCPUTime[-1]
        ...
        if not p.userStop and (not condEqualLastPoints or p.probType == 'GLP' or p.solver.__name__ in ('de', 'pswarm', 'interalg')):
            ...
            if IS_NAN_IN_X in p.stopdict:pass
            elif SMALL_DELTA_X in p.stopdict and array_equal(p.iterValues.x[-1], p.iterValues.x[-2]): pass
            else:
                p.nonStopMsg = ''
